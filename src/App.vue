<template>
  <el-container v-if="authStore.token" :class="['h-screen']" class="p-0">
    <el-header class="header text-white overflow-hidden p-0 bg-gray-800 shadow"
          ...
          <el-dropdown v-if="authStore.profile" class="ml-7">
            ...
            <template #dropdown>
              <el-dropdown-menu style="width: 160px;">
                <el-dropdown-item disabled class="text-dark">
                  {{ authStore.profile.name }} <small> ({{ authStore.profile.mobile }})</small>
                </el-dropdown-item>
  ...
  <el-container v-else class="h-screen">
    <el-aside class="auth-aside aside" width="320px"></el-aside>
...
.auth-aside {
  background-color: rgba(1, 1, 119, 0.7) !important;
