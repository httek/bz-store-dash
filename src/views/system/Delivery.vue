<template>
  <div class="container-full w-full p-4">
    ...
    <el-table :border="true" stripe v-if="items.data.length" :data="items.data" row-key="id" highlight-current-row
      table-layout="auto">
    ...
    <div class="flex justify-end w-full mt-4" v-if="items.data.length">
      <el-pagination @current-change="(page) => { items.page = page; getDeliveryList() }" class="justify-end"
...
const getDeliveryList = () => {
  DeliveryAPIs.list(items.page, items.size, filterForm).then(res => Object.assign(items, res.data))
}
