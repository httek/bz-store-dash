<template>
  <div class="container-full w-full p-4">
    ...
    <el-table style="width: 100% !important;" :border="true" stripe v-if="items.data.length" :data="items.data" row-key="id" highlight-current-row >
      <el-table-column prop="id" width="80" label="ID" />
      <el-table-column prop="name" min-width="100" label="商户" />
      <el-table-column prop="store_name" width="65" label="店铺">
        <template #default="scope">
          <el-tooltip class="box-item" effect="dark" :content="scope.row.store_name"
            placement="right">
            <el-avatar shape="square" :size="36" :src="scope.row.store_logo">{{ scope.row.store_name.substring(0, 2) }}</el-avatar>
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column width="100" prop="cash_type" label="提现模式">
        <template #default="scope">
          <el-tag :type="scope.row.cash_type === 1 ? 'success' : ''" disable-transitions>{{ scope.row.cash_type ? '对私' : '对共'
            }}
          </el-tag>
        </template>
      </el-table-column>

      <el-table-column width="100" prop="delivery_template" label="配送方式">
        <template #default="scope">
          <el-tooltip :disabled="!scope.row.delivery_template" class="box-item" effect="dark" :content="scope.row.delivery_template?.tips"
                      placement="right">
      ...
      <el-table-column prop="aptitude" width="100" label="资质文件">
        <template #default="scope">
...
const opFormModelInit = { type: 0, name: '', sequence: 0, status: 1, id: 0 }
